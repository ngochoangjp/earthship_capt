...
def create_user_interface():
    ...
    with gr.Blocks(css="""
        ...
        with gr.Group(visible=False) as chat_group:
            with gr.Row():
                # --- User Profile ---
                with gr.Column(scale=1, elem_classes="profile-column"):
                    gr.Markdown("## Thông tin cá nhân", elem_classes="profile-header")
                    gr.Markdown("AI lưu lại thông tin của bạn chỉ để hiểu bạn hơn.", elem_classes="profile-description")
                    real_name = gr.Textbox(label="Họ và tên", placeholder="Nhập tên của bạn", elem_classes="profile-input")
                    age = gr.Number(label="Tuổi", elem_classes="profile-input")
                    gender = gr.Textbox(label="Giới tính", placeholder="Nhập giới tính của bạn", elem_classes="profile-input")
                    vegan_checkbox = gr.Checkbox(label="Ăn chay", value=False, interactive=True, elem_classes="profile-input")
                    height = gr.Number(label="Chiều cao (cm)", elem_classes="profile-input")
                    weight = gr.Number(label="Cân nặng (kg)", elem_classes="profile-input")
                    muscle_percentage = gr.Textbox(label="Phần trăm cơ (%)", placeholder="Nhập phần trăm cơ", elem_classes="profile-input")
                    passion = gr.Textbox(label="Đam mê, sở thích", placeholder="Bạn thích làm điều gì?", elem_classes="profile-input")
                    job = gr.Textbox(label="Nghề nghiệp", placeholder="Nhập nghề nghiệp của bạn", elem_classes="profile-input")
                    personality_text = gr.TextArea(label="Tính cách", placeholder="Mô tả tính cách của bạn", elem_classes="profile-input")
                    save_profile = gr.Button("Lưu thông tin", variant="primary", elem_classes="profile-button")
                    hide_profile_button = gr.Button("Ẩn thông tin cá nhân", elem_classes="profile-button")
                    show_profile_button = gr.Button("Chỉnh sửa thông tin cá nhân", visible=False, elem_classes="profile-button")
                    password_input = gr.Textbox(label="Nhập mật khẩu", type="password", visible=False, elem_classes="profile-input")
                    password_error = gr.Markdown(visible=False, elem_classes="profile-error")

                # --- Chat and Inputs ---
                with gr.Column(scale=3, elem_classes="chat-column"):
                    personality = gr.Dropdown(choices=list(PERSONALITIES.keys()), value=list(PERSONALITIES.keys())[0], label="Chọn tính cách AI", interactive=True, elem_classes="chat-dropdown")
                    chatbot = gr.Chatbot(
                        elem_id="chatbot",
                        height=600,
                        render=True,
                ...
                with gr.Column(scale=1, elem_classes="settings-column"):
                    model = gr.Dropdown(choices=list(MODEL_DISPLAY_NAMES.keys()), value=list(MODEL_DISPLAY_NAMES.keys())[0], label="Chọn mô hình AI", interactive=True, elem_classes="settings-dropdown")
                    use_internet_checkbox = gr.Checkbox(label="Sử dụng Internet để tìm kiếm", value=False, elem_classes="settings-checkbox")
                    new_chat_button = gr.Button("Bắt đầu cuộc trò chuyện mới", elem_classes="settings-button")
                    gr.Markdown("### Thư viện công cụ", elem_classes="settings-header")
                    premade_prompt_buttons = [gr.Button(prompt_name, elem_classes="settings-button") for prompt_name in PREMADE_PROMPTS.keys()]
                    gr.Markdown("## Lịch sử trò chuyện", elem_classes="settings-header")
                    chat_history_dropdown = gr.Dropdown(choices=[], label="Chọn lịch sử trò chuyện", interactive=True, elem_classes="settings-dropdown")
                    with gr.Row():
                        load_chat_button = gr.Button("Load Chat", elem_classes="settings-button")
                        rename_chat_button = gr.Button("Rename Chat", elem_classes="settings-button")
                    rename_chat_textbox = gr.Textbox(placeholder="Enter new chat name", visible=False, elem_classes="settings-input")
import os
import re
import json
import base64
